...
// awsSubnetReservedPrefixes returns the /28 prefixes that contain the AWS reserved IPs for a given subnet
func awsSubnetReservedPrefixes(subnet netip.Prefix) (reservedPrefixes []netip.Prefix) {
	if bits := subnet.Bits(); bits >= 28 {
		// subnet is too small to contain /28s
		return nil
	}
	// Returns the first and last /28s of the subnet
	first, _ := eniPDPrefix(subnet.Masked().Addr())
	last, _ := eniPDPrefix(netipx.PrefixLastIP(subnet))
	// TODO(hadrien): both above lines return an error, log it or return it
	return []netip.Prefix{first, last}
}
