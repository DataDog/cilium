...
func (a *addressesManager) FindIPs(subnet netip.Prefix, addressesCount int32) (addresses []string, found bool) {
	...
	// TODO: add affinity to the prefixes that are already closest to being full
	...
	for prefix, prefixUsage := range *a.getPrefixesUsage(subnet) {
		...
		log.Warnf("HADRIEN 1 prefix %s availableIPsInPrefix %d", prefix, availableIPsInPrefix)
	...
	for i, prefixesUsage := range prefixesByAvailableIPs {
		log.Warnf("HADRIEN 5 i %d prefixesUsage %v", i, prefixesUsage)
		if prefixesUsage == nil {
			continue
		}
		for prefix, prefixUsage := range *prefixesUsage {
			log.Warnf("HADRIEN 6 prefix %v prefixUsage %v", prefix, prefixUsage)
