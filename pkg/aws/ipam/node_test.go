...
func TestEffectiveENAQueueCount(t *testing.T) {
	// c8i.8xlarge assigns 8 queues per interface by default and allows up to 32.
	limits := ipamTypes.Limits{
		SupportsFlexibleENAQueues:        true,
		MaxENAQueueCount:                 128,
		MaxENAQueueCountPerInterface:     32,
	...
	tests := []struct {
		...
		{
			// The default is reported for every instance type, including the
			// ones which do not allow the count to be chosen.
			name:     "the default applies without flexible queue support too",
			eni:      types.ENI{ID: "eni-0", Number: 0},
			limits:   ipamTypes.Limits{DefaultENAQueueCountPerInterface: 8},
			expected: 8,
		},
		{
			name:     "unknown limits report no count",
			eni:      types.ENI{ID: "eni-0", Number: 0},
			limits:   ipamTypes.Limits{},
			expected: 0,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			require.Equal(t, tt.expected, effectiveENAQueueCount(&tt.eni, tt.limits))
		})
	}
}
