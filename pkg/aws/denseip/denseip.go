...
func RegisterIPUsed(ip string, subnet netip.Prefix) {
	addr, err := netip.ParseAddr(ip)
	if err != nil {
		log.WithError(err).Debugf("Failed to parse IP %s", ip)
	}
	if err := subnetsUsage.registerAddrUsed(addr, subnet); err != nil {
		log.WithError(err).Debugf("Failed to register IP %s as used", ip)
	}
}

func RegisterIPsUsed(ips []string, subnet netip.Prefix) {
	for _, ip := range ips {
		RegisterIPUsed(ip, subnet)
	}
}

func RegisterIPUnused(ip string) {
	addr, err := netip.ParseAddr(ip)
	if err != nil {
		log.WithError(err).Debugf("Failed to parse IP %s", ip)
	}
	if err := subnetsUsage.registerAddrUnused(addr); err != nil {
		log.WithError(err).Debugf("Failed to register IP %s as unused", ip)
	}
}

func RegisterIPsUnused(ips []string) {
	for _, ip := range ips {
		RegisterIPUnused(ip)
	}
}
