...
func NewLimitsGetter(api ec2API, triggerMinInterval, ec2APITimeout time.Duration, ec2APIRetryCount int) (*LimitsGetter, error) {
	...
	if err != nil {
		log.WithError(err).Error("Unable to initialize EC2 API update trigger")
		return nil, err
	}
	return l, nil
}
	...
	case <-currentDone:
		...
		log.WithFields(
			logrus.Fields{
				logfields.Limit:        limit,
				logfields.InstanceType: instanceType,
			}).Debug("Get limits from cache after update")
		return limit, ok
	case <-time.After(ec2apiTimeoutWithJitter):
		log.WithFields(
			logrus.Fields{
				logfields.InstanceType: instanceType,
			}).Debug("Failed to get instance limits from EC2 API after timeout")
		return ipamTypes.Limits{}, false
	}
}
	...
	return trigger.NewTrigger(trigger.Parameters{
		...
		TriggerFunc: func(reasons []string) {
			if err := l.updateFromEC2APIWithRetry(context.TODO(), api, timeout, retryCount); err != nil {
				log.Error("Failed to update instance limits from EC2 API", logfields.Error, err)
			}
...
func (l *LimitsGetter) initEC2APIUpdateTrigger(api ec2API, triggerMinInterval, ec2APITimeout time.Duration, ec2APIRetryCount int) error {
	...
	if err != nil {
		log.WithError(err).Error("Failed to create EC2 update trigger")
		return err
	}
	...
	if err := l.updateFromEC2APIWithRetry(context.Background(), api, ec2APITimeout, ec2APIRetryCount); err != nil {
		log.WithError(err).Error("Failed initial EC2 API limits update")
		return err
	}
