...
func AssignedIP(ip string, subnet netip.Prefix) error {
	...
	if !ok {
		...
		// This is the first time recording information about this subnet, record AWS reserved IPs as used
		reservedAddr := subnet.Masked().Addr()
		for i := 0; i < 3; i++ {
			if err := AssignedIP(reservedAddr.String(), subnet); err != nil {
				return err
			}
			reservedAddr = reservedAddr.Next()
		}
		//TODO: also register the broadcast address as used
	}
	...
	logHADRIEN.WithField("subnetsUsage", subnetsUsage).Infof("Registered: Assigned IP %s from prefix %s in subnet %s", addr, prefix, subnet)
	return nil
}
...
func UnassignedIP(ip string) error {
	...
	prefixesUsage[prefix] = prefixUsage
	subnetsUsage[subnet] = prefixesUsage
	logHADRIEN.WithField("subnetsUsage", subnetsUsage).Infof("Registered: Unassigned IP %s from prefix %s in subnet %s", addr, prefix, subnet)
	return nil
}
...
func FindIPs(subnet netip.Prefix, addressesCount int32) (addresses []string, found bool) {
	...
	logHADRIEN.WithField("subnetsUsage", subnetsUsage).Infof("Found potentially available %d IPs in subnet %s: %v", len(addresses), subnet, addresses)
