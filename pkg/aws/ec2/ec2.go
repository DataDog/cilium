...
func (c *Client) CreateNetworkInterface(ctx context.Context, toAllocate int32, subnet ipamTypes.Subnet, desc string, groups []string, allocatePrefixes bool) (string, *eniTypes.ENI, error) {
	...
	if err != nil && foundIPs {
		// We failed to assign found IPs, falling back to assigning random IPs
		// TODO: log err (the first error)
...
func (c *Client) AssignPrivateIpAddresses(ctx context.Context, eniID string, addressesCount int32, subnet netip.Prefix) error {
	...
	if output != nil {
		for _, ip := range output.AssignedPrivateIpAddresses {
			assignedAdresses = append(assignedAdresses, aws.ToString(ip.PrivateIpAddress))
		}
	}
