...
func (c *Client) describeNetworkInterfaces(ctx context.Context, subnets ipamTypes.SubnetMap) ([]ec2_types.NetworkInterface, error) {
	...
	for paginator.HasMorePages() {
		...
		span2, ctx := tracer.StartSpanFromContext(
			ctx,
			"pkg/aws/ec2",
			tracer.ResourceName("(*DescribeNetworkInterfacesPaginator).NextPage"),
...
func (c *Client) AssociateEIP(ctx context.Context, eniID string, eipTags ipamTypes.Tags) (string, error) {
	...
	for _, address := range addresses.Addresses {
		...
		if err != nil {
			err := fmt.Errorf("failed to associate EIP %s (AllocationId: %s, AssociationId: %s, InstanceId: %s, NetworkInterfaceId: %s) with ENI %s: %w", aws.String(address.PublicIp), aws.String(address.AllocationId), aws.String(address.AssociationId), aws.String(address.InstanceId), aws.String(address.NetworkInterfaceId), eniID, err)
		}
