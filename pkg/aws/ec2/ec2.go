...
// addresses
func (c *Client) AssignPrivateIpAddresses(ctx context.Context, eniID string, addresses int32, subnet string) error {
	...
	if addresses == 1 {
		...
		if ok {
			...
			if errors.As(err, &apiErr) {
				logHADRIEN.Errorf("Smart AssignPrivateIpAddresses failed to assign IP %s for ENI %s:\n%s\n%s\n%v", addr, eniID, apiErr.ErrorCode(), apiErr.ErrorMessage(), apiErr.ErrorFault())
