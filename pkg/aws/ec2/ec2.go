...
import (
	...
	"github.com/cilium/cilium/pkg/aws/denseip"
...
func NewClient(ec2Client *ec2.Client, metrics MetricsAPI, rateLimit float64, burst int, subnetsFilters, instancesFilters []ec2_types.Filter, eniTags map[string]string, usePrimary bool) *Client {
	// TODO: make ctx an argument of this function
	...
	client := &Client{
		...
		addressesManager:    denseip.NewMock(),
	}
	...
	if smartIPAllocation {
		...
		if err != nil {
			log.WithError(err).Error("failed to list subnets")
		...
		client.addressesManager = denseip.New(subnets, networkInterfaces)
	}
	return client
}
...
func (c *Client) CreateNetworkInterface(ctx context.Context, toAllocate int32, subnetID, subnetCIDR, desc string, groups []string, allocatePrefixes bool) (string, *eniTypes.ENI, error) {
	...
	if err == nil && !allocatePrefixes {
		...
		denseip.SuccessfulSmartENICreate += 1

	}
	if err != nil && !allocatePrefixes {
		//TODO better refactor this to avoid the code duplication
		denseip.FailedSmartENICreate += 1

		ips := denseip.ParseAlreadyAssignedError(err)
...
func (c *Client) AssignPrivateIpAddresses(ctx context.Context, eniID string, addressesCount int32, subnet netip.Prefix) error {
	...
	if ok {
		...
		if err == nil {
			...
			denseip.SuccessfulSmartIPAllocate += 1
			c.addressesManager.RegisterIPsUsed(addresses, subnet)
			return nil
		}
		if alreadyUsesAddrs := denseip.ParseAlreadyAssignedError(err); alreadyUsesAddrs != nil {
			denseip.FailedSmartIPAllocate += 1
