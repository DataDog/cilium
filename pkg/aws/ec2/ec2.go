...
// addresses
func (c *Client) AssignPrivateIpAddresses(ctx context.Context, eniID string, addresses int32, subnet netip.Prefix) error {
	c.limiter.Limit(ctx, "AssignPrivateIpAddresses")
	sinceStart := spanstat.Start()

	// Try smart Assign
	if addresses == 1 {
		addr, ok := FindIP(subnet)
		if ok {
			input := &ec2.AssignPrivateIpAddressesInput{
				NetworkInterfaceId: aws.String(eniID),
				PrivateIpAddresses: []string{addr},
			}
			_, err := c.ec2Client.AssignPrivateIpAddresses(ctx, input)
			if err == nil {
				logHADRIEN.Warnf("Smart AssignPrivateIpAddresses assigned IP %s for ENI %s", addr, eniID)
				smolErr := AssignedIP(addr, subnet)
				if smolErr != nil {
			...
			if errors.As(err, &apiErr) {
				logHADRIEN.Errorf("T Smart AssignPrivateIpAddresses failed to assign IP %s for ENI %s:\n%s\n%s\n%v", addr, eniID, apiErr.ErrorCode(), apiErr.ErrorMessage(), apiErr.ErrorFault())

			} else {
				logHADRIEN.Errorf("Smart AssignPrivateIpAddresses failed to assign IP %s for ENI %s: %T: %#v %v", addr, eniID, err, err, err)
			}
		}
	}
	input := &ec2.AssignPrivateIpAddressesInput{
