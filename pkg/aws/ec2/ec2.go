...
func (c *Client) CreateNetworkInterface(ctx context.Context, toAllocate int32, subnetID, subnetCIDR, desc string, groups []string, allocatePrefixes bool) (string, *eniTypes.ENI, error) {
	...
	if err != nil {
		//TODO: on create failure because of already used IPs, retry to create interface with random IPs
		// sample error to catch:
		// operation error EC2: CreateNetworkInterface, https response error StatusCode: 400, RequestID: 48657cad-8598-4c4d-96e1-c2e436cce7b5, api error InvalidParameterValue: There aren't sufficient free Ipv4 addresses or allocatePrefixes

		logHADRIEN.WithError(err).Errorf("HADRIEN failed to create ENI with found IPs, falling back to random IPs")

		var opErr *smithy.OperationError
		if errors.As(err, &opErr) {
			logHADRIEN.WithField("ServiceID", opErr.Service()).
				WithField("OperationName", opErr.Operation()).
				WithField("Error", opErr.Unwrap()).
				Warnf("HADRIEN opErr here")
		}
		var apiErr *smithy.APIError
		if errors.As(err, &apiErr) {
			logHADRIEN.WithField("ErrorCode", apiErr.ErrorCode()).
				WithField("ErrorMessage", apiErr.ErrorMessage()).
				Warnf("HADRIEN apiErr here")
		}
		...
		// 	var apiErr smithy.APIError
		// 	wrappedErr := opErr.Unwrap()
		// 	if errors.As(wrappedErr, &apiErr) {
		// 		log.WithField("ErrorCode", apiErr.ErrorCode()).
		// 			WithField("ErrorMessage", apiErr.ErrorMessage()).
		// 			WithField("ErrorFault", apiErr.ErrorFault()).
		// 			Warnf("HADRIEN apiErr here")
		...
		// 	log.WithError(awsErr).Errorf("HADRIEN code %v message %v", awsErr.Code(), awsErr.Message())
		// }
		if err != nil {
			return "", nil, err
		}
