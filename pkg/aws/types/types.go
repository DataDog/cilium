...
type ENISpec struct {
	...
	// Cilium attaches to this node. It accepts "default", "auto" or a number of
	// queues, which AWS requires to be a power of two. "default", which is the
	// value the agent writes when nothing is configured, leaves the number of
	// queues to AWS.
	//
	// "auto" requests as many queues as the interface can use, that is the
	// number of vCPUs of the instance rounded up to a power of two, as AWS only
	// accepts a power of two, capped by the maximum number of queues per
	// interface of the instance type. Rounding up rather than down keeps one
	// queue per CPU, as network drivers do not create more queues than there
	// are CPUs.
	//
	...
	//
	// +kubebuilder:validation:Pattern=`^(default|auto|1|2|4|8|16|32|64|128|256|512|1024)$`
	// +kubebuilder:validation:Optional
...
type ENI struct {
	...
	//
	// The EC2 API only reports a queue count for attachments which requested
	// one, so for any other attachment, including the primary interface and
	// interfaces Cilium does not manage, this is the default number of queues
	// of the instance type. It is zero only while the limits of the instance
	// type are unknown.
	//
	// +optional
