	...
	go func() {
		...
		if ok {
			...
			if regenError != nil && !errors.Is(regenError, context.Canceled) {
				...
				e.getLogger().Info("Context ctx error: ", ctx.Err(), "context address: "+fmt.Sprintf("%p", ctx))
		...
		if !buildSuccess && !canceled {
			e.getLogger().WithError(regenError).Info("Anton-Test: Regenerate failed for regenMetadata "+regenMetadata.Reason, " (with buildSuccess="+strconv.FormatBool(buildSuccess)+" and canceled="+strconv.FormatBool(canceled)+"). Retrying")
			select {
