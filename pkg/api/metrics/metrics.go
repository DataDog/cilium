...
var log = logging.DefaultLogger.WithField(logfields.LogSubsys, "HADRIEN")
...
func (p *PrometheusMetrics) ObserveAPICall(operation, status string, duration float64) {
	log.WithField("operation", operation).WithField("duration", duration).Debugf("HADRIEN: ObserveAPICall called for %s in %vs", operation, duration)
