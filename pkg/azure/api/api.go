...
func (c *Client) vmNetworkInterfaces(ctx context.Context) ([]armnetwork.Interface, error) {
	...
	defer c.metricsAPI.ObserveAPICall(interfacesListComplete, deriveStatus(err), sinceStart.Seconds())
...
func (c *Client) vmssNetworkInterfaces(ctx context.Context) ([]armnetwork.Interface, error) {
	...
	defer c.metricsAPI.ObserveAPICall(virtualMachineScaleSetsListAll, deriveStatus(err), sinceStart.Seconds())
	for vmscalesetsPager.More() {
		...
		for _, scaleset := range nextResult.Value {
			...
			defer c.metricsAPI.ObserveAPICall(interfacesListVirtualMachineScaleSetNetworkInterfacesComplete, deriveStatus(err2), sinceStart.Seconds())
			for interfacesPager.More() {
...
func (c *Client) describeVpcs(ctx context.Context) ([]armnetwork.VirtualNetwork, error) {
	...
	defer c.metricsAPI.ObserveAPICall(virtualnetworksListAll, deriveStatus(err), sinceStart.Seconds())
