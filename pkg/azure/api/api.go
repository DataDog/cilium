...
func (c *Client) vmssNetworkInterfaces(ctx context.Context) ([]armnetwork.Interface, error) {
	...
	var vmscalesets []armcompute.VirtualMachineScaleSet
	func(vmscalesets []armcompute.VirtualMachineScaleSet) ([]armcompute.VirtualMachineScaleSet, error) {
		...
		for vmscalesetsPager.More() {
			...
			for _, scaleset := range nextResult.Value {
				...
				vmscalesets = append(vmscalesets, *scaleset)
	...
	var networkInterfaces2 []armnetwork.Interface
	for _, scaleset := range vmscalesets {

		err := func(networkInterfaces *[]armnetwork.Interface) error {
