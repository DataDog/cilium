...
func NewClient(cloudName, subscriptionID, resourceGroup, userAssignedIdentityID string, metrics MetricsAPI, rateLimit float64, burst int, usePrimary bool) (*Client, error) {
	...
	vmssClient, err := armcompute.NewVirtualMachineScaleSetVMsClient(subscriptionID, credential, armClientOptions)
	if err != nil {
		return nil, err
	}

	vmscalesetsClient, err := armcompute.NewVirtualMachineScaleSetsClient(subscriptionID, credential, armClientOptions)
	if err != nil {
		return nil, err
	}
...
func (c *Client) vmssNetworkInterfaces(ctx context.Context) ([]armnetwork.Interface, error) {
	...
	for vmscalesetsPager.More() {
		...
		for _, scaleset := range nextResult.Value {
			...
			networkInterfaces, err = func(networkInterfaces []armnetwork.Interface) ([]armnetwork.Interface, error) {
				...
				defer func() {
					c.metricsAPI.ObserveAPICall(interfacesListVirtualMachineScaleSetNetworkInterfaces, deriveStatus(err2), sinceStart.Seconds())
				}()
				for interfacesPager.More() {
...
// describeVpcs lists all VPCs (not just those in
func (c *Client) describeVpcs(ctx context.Context) ([]armnetwork.VirtualNetwork, error) {
	...
	virtualnetworksPager := c.virtualnetworks.NewListAllPager(nil)
	defer func() {
...
func (c *Client) AssignPrivateIpAddressesVMSS(ctx context.Context, instanceID, vmssName, subnetID, interfaceName string, addresses int) error {
	...
	instanceViewType := armcompute.InstanceViewTypesInstanceView
	vmssGetOptions := &armcompute.VirtualMachineScaleSetVMsClientGetOptions{
		Expand: &instanceViewType,
	}

	c.limiter.Limit(ctx, virtualMachineScaleSetVMsGet)
	sinceStart := spanstat.Start()
	result, err := c.vmss.Get(ctx, c.resourceGroup, vmssName, instanceID, vmssGetOptions)
