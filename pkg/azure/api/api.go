...
func (c *Client) getPublicIPPrefixIDByTags(ctx context.Context, searchTags ipamTypes.Tags) (string, error) {
	log.Info("Searching for public IP prefix with tags %v", searchTags)
	...
	for result.NotDone() {
		...
		if publicIpPrefix.ID == nil {
			log.Warn("Found public IP prefix with tags %v but missing ID", searchTags)
			continue
		}
		log.Info("Found public IP prefix with tags %v and ID %s", searchTags, *publicIpPrefix.ID)
