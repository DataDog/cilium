...
func NewKVStoreBackend(c KVStoreBackendConfiguration) (*KVStoreBackend, error) {
	...
	return &KVStoreBackend{
		...
		suffix:      c.Suffix,
		keyType:     c.Typ,
		lockless:    locklessCapability(c.Backend),
		backend:     c.Backend,
	}, nil
}
...
func (k *KVStoreBackend) RunGC(
	...
	for key, v := range allocated {
		...
		identityID, err := strconv.ParseUint(items[len(items)-1], 10, 64)
		if err != nil {
		...
		hasUsers := false
		// Verify that the key is used and is actually assigned to the right identity ID
		for prefix, referencedId := range pairs {
			referencedIdParsed, err := strconv.ParseUint(string(referencedId.Data), 10, 64)
			if err != nil {
				log.WithField(fieldID, referencedId).WithError(err).Warning("Parsing of referenced identity ID failed, skipping")
				continue
			}
			if prefixMatchesKey(valueKeyPrefix, prefix) && referencedIdParsed == identityID {
				hasUsers = true
				break
			}
		}
