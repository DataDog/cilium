...
func connectEtcdClient(ctx context.Context, config *client.Config, cfgPath string, errChan chan error, clientOptions clientOptions, opts *ExtraOptions) (BackendOperations, error) {
	...
	ec.logger.WithField("X_opts", fmt.Sprintf("%+v", opts)).Info("Connecting to etcd server 2 [[[CAN YOU SEE ME????]]]...")
	ec.logger.WithField("X_config", fmt.Sprintf("%+v", config)).Info("Connecting to etcd server 3 [[[CAN YOU SEE ME????]]]...")
