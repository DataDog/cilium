...
func (m *InstancesManager) Resync(ctx context.Context) time.Time {
	...
	}).Info("Synchronized Oracle IPAM information")
...
func (m *InstancesManager) FindSubnetByTags(vcnID, availabilityDomain string, required ipamTypes.Tags) (bestSubnet *ipamTypes.Subnet) {
	m.mutex.RLock()
	defer m.mutex.RUnlock()

	for _, s := range m.subnets {
		if s.VirtualNetworkID == vcnID && s.AvailabilityZone == availabilityDomain && s.Tags.Match(required) {
