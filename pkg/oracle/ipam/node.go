...
func (n *Node) PrepareIPRelease(excessIPs int, scopedLog *logrus.Entry) *ipam.ReleaseAction {
	...
	err := n.manager.instances.ForeachInterface(n.node.InstanceID(), func(instanceID, interfaceID string, interfaceObj ipamTypes.InterfaceRevision) error {
		...
		ipsToRelease := math.IntMin(len(freeIps), excessIPs)
		r.InterfaceID = iface.ID
		r.PoolID = ipamTypes.PoolID(iface.SubnetID)
		r.IPsToRelease = freeIps[:ipsToRelease]
...
func (n *Node) ReleaseIPs(ctx context.Context, r *ipam.ReleaseAction) error {
	// TODO ip IDs need to be tracked in a map for faster lookup
	// Also missing released ips are not handled right now. Errors need to be handled too
	for _, ipToRelease := range r.IPsToRelease {
		n.manager.instances.ForeachAddress(n.node.InstanceID(), func(instanceID, interfaceID, ip, poolID string, address ipamTypes.Address) error {
			if ipToRelease == ip {
				a := address.(*types.OracleIP)
				n.manager.api.DeletePrivateIpAddress(ctx, a.ID)
			}
			return nil
		})
	}
...
func (n *Node) CreateInterface(ctx context.Context, allocation *ipam.AllocationAction, scopedLog *logrus.Entry) (int, string, error) {
	...
	securityGroupIDs, err := n.getSecurityGroupIDs(n.k8sObj.Spec.Oracle)
	if err != nil {
		return 0,
			"unableToGetSecurityGroups", // TODO const
			fmt.Errorf("unableToGetSecurityGroups: %w", err)
	}
	...
	vnic, err := n.manager.api.AttachVnic(ctx, n.node.InstanceID(), *subnet, securityGroupIDs)
	if err != nil {
...
func (n *Node) getSecurityGroupIDs(oracleSpec types.OracleSpec) ([]string, error) {
	if len(oracleSpec.SecurityGroupIds) > 0 {
		return oracleSpec.SecurityGroupIds, nil
	}

	/*
		if len(oracleSpec.SecurityGroupTags) > 0 {
			// TODO
		}
	*/
	...
	n.manager.instances.ForeachInterface(n.node.InstanceID(),
		func(instanceID, interfaceID string, interfaceObj ipamTypes.InterfaceRevision) error {
			iface, ok := interfaceObj.Resource.(*types.OracleInterface)
			if ok && iface.IsPrimary {
				securityGroups = append(securityGroups, iface.SecurityGroupIds...)
			}
			return nil
		})
