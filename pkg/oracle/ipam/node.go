...
func (n *Node) PrepareIPAllocation(scopedLog *logrus.Entry) (a *ipam.AllocationAction, err error) {
	...
	// TODO calculate the max number of secondary interfaces that can be allocated
	if numInterfaces == 1 {
...
func (n *Node) AllocateIPs(ctx context.Context, a *ipam.AllocationAction) error {
	iface, ok := a.Interface.Resource.(*types.OracleInterface)
	if !ok {
		return fmt.Errorf("invalid interface object")
	}

	for i := 0; i < a.IPv4.AvailableForAllocation; i++ {
		_, err := n.manager.api.AssignPrivateIpAddress(ctx, iface.ID)
		if err != nil {
			return err
		}
	}
...
func (n *Node) CreateInterface(ctx context.Context, allocation *ipam.AllocationAction, scopedLog *logrus.Entry) (int, string, error) {
	vcnID := n.k8sObj.Spec.Oracle.VcnID
	availabilityDomain := n.k8sObj.Spec.Oracle.AvailabilityDomain
	subnetTags := n.k8sObj.Spec.Oracle.SubnetTags

	subnet := n.manager.FindSubnetByTags(vcnID, availabilityDomain, subnetTags)
	if subnet == nil {
		return 0,
			"unable to find subnet", // TODO make this a const?
			fmt.Errorf(
				"No matching subnet available for interface creation (VNC ID=%s AvailabilityDomain=%s SubnetTags=%s)",
				vcnID,
				availabilityDomain,
				subnetTags,
			)
	...
	vnic, err := n.manager.api.AttachVnic(ctx, n.node.InstanceID(), subnet.ID)
	if err != nil {
