...
// connectivity. An error is therefore returned if the configured native
// routing CIDR overlaps none of the VPC CIDRs.
func autoDetectENINativeRoutingCIDR(
	...
	if nativeCIDR := conf.IPv4NativeRoutingCIDR; nativeCIDR != nil {
		...
		if !ok {
			return fmt.Errorf("invalid --%s %s", option.IPv4NativeRoutingCIDR, nativeCIDR)
		}
		...
		if !slices.ContainsFunc(vpcCIDRs, func(c netip.Prefix) bool {
			return iputil.LaminarCIDRsOverlap(native, c)
		}) {
...
func newENIMultiPoolAllocators(ctx context.Context, p ENIMultiPoolAllocatorParams) (Allocator, Allocator, error) {
	...
	if len(vpcCIDRs) == 0 {
		return nil, nil, errors.New("no IPv4 VPC CIDRs reported for the node")
	}
