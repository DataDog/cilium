...
not_esp:
...
#if defined(ENABLE_EGRESS_GATEWAY_COMMON)
	{
		...
		if (egress_gw_snat_needed_hook(ip4->saddr, daddr, &snat_addr,
			...
			cilium_dbg(ctx, DBG_GENERIC, 0x6001, egress_ifindex); // DEBUG: OVERLAY egress gateway redirect
...
int tail_handle_arp(struct __ctx_buff *ctx)
{
	...
	if (info->tunnel_endpoint) {
		cilium_dbg(ctx, DBG_GENERIC, 0x6002, info->tunnel_endpoint); // DEBUG: OVERLAY VTEP redirect
