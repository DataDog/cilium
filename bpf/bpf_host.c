 ...
 */
	...
	if (info && info->tunnel_endpoint != 0) {
		cilium_dbg(ctx, DBG_REDIRECT, 0x5002, 0); // DEBUG: HOST tunnel redirect
		 ...
		 __s8 *ext_err __maybe_unused)
{
...
#ifdef ENABLE_VTEP
	{
		...
		if (vtep->vtep_mac && vtep->tunnel_endpoint) {
			cilium_dbg(ctx, DBG_REDIRECT, 0x5001, 0); // DEBUG: HOST VTEP redirect
...
skip_vtep:
	...
	if (info && info->tunnel_endpoint != 0) {
		...
		cilium_dbg(ctx, DBG_REDIRECT, 0x5003, 0); // DEBUG: HOST netdev encap redirect
		ret = encap_and_redirect_netdev(ctx, &key, encrypt_key, secctx, &trace);
		if (ret != DROP_NO_TUNNEL_ENDPOINT)
			return ret;
	}
...
# endif /* ENABLE_IPV4 */
	...
	ctx->mark = 0;
	bpf_clear_meta(ctx);
	cilium_dbg(ctx, DBG_REDIRECT, 0x5004, 0); // DEBUG: HOST encrypt encap redirect
...
int cil_to_host(struct __ctx_buff *ctx)
{
	...
	if ((magic & 0xFFFF) == MARK_MAGIC_TO_PROXY) {
		...
		cilium_dbg(ctx, DBG_REDIRECT, 0x5005, 0); // DEBUG: HOST proxy redirect
	 ...
	 */
	if (ctx->mark == 0 && THIS_INTERFACE_IFINDEX == HOST_IFINDEX)
		ctx->mark = MARK_MAGIC_PROXY_TO_WORLD;
#endif /* !TUNNEL_MODE */
