...
/* Forward an IPv4 packet to its destination, following the same path as if
 * the packet was genuinely sent out by a pod. This handles:
 * - SRV6 encapsulation
 * - L7 proxy redirection
 * - Host firewall ingress policies
 * - Local endpoint delivery (hairpin flows)
 * - VTEP redirection
...
#if defined(TUNNEL_MODE)
		ct_state_local.from_tunnel = from_tunnel;
#endif

		/* Lookup destination to determine if we need tunnel encapsulation.
		 * The ICMP reply must follow the same forwarding path as if it was
		 * a regular packet sent by the pod.
		 */
