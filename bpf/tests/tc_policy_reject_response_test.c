...
static __always_inline void init_policy_reject_response_setup(struct __ctx_buff *ctx, __be32 saddr, __be32 daddr)
{
	/* Add ipcache entries */
	ipcache_v4_add_entry(saddr, 0, 112233, 0, 0);
	ipcache_v4_add_entry(daddr, 0, 445566, 0, 0);

	/* Add policy that denies egress to target */
	policy_add_egress_deny_all_entry();
}

static __always_inline void check_policy_reject_response(const struct __ctx_buff *ctx, __be32 saddr, __be32 daddr)
{
	void *data, *data_end;
	__u32 *status_code;
	struct iphdr *l3;
...
{
	init_policy_reject_response_setup(ctx, CLIENT_IP, TARGET_IP);
...
{
	test_init();
	check_policy_reject_response(ctx, CLIENT_IP, TARGET_IP);
	test_finish();
}
...
{
	return build_packet(ctx, INGRESS_SRC_IP, INGRESS_DST_IP);
}

/* Test policy deny response on ingress to an endpoint from a world identity
 * The return should be an ICMP packet filtered from INGRESS_DST_IP -> INGRESS_SRC_IP
 */
SETUP("tc", "policy_reject_response_ingress_v4")
int policy_reject_response_ingress_setup(struct __ctx_buff *ctx)
{
	init_policy_reject_response_setup(ctx, INGRESS_SRC_IP, INGRESS_DST_IP);
...
int policy_reject_response_ingress_check(const struct __ctx_buff *ctx)
{
	...
	check_policy_reject_response(ctx, INGRESS_SRC_IP, INGRESS_DST_IP);
