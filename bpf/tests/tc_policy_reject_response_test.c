...
#define CLIENT_IP_V6 v6_pod_one
#define TARGET_IP_V6 v6_ext_node_one
...
int policy_reject_response_v6_pktgen(struct __ctx_buff *ctx)
{
	...
	l4 = pktgen__push_ipv6_tcp_packet(&builder,
					  (__u8 *)mac_one, (__u8 *)mac_two,
					  (__u8 *)CLIENT_IP_V6, (__u8 *)TARGET_IP_V6,
					  tcp_src_one, tcp_dst_one);
	if (!l4)
		return TEST_ERROR;
...
{
	/* Add endpoint for source */
	endpoint_v6_add_entry((union v6addr *)CLIENT_IP_V6, 0, 0, 0, 0, NULL, NULL);
	
	/* Add ipcache entries */
	ipcache_v6_add_entry((union v6addr *)CLIENT_IP_V6, 0, 112233, 0, 0);
	ipcache_v6_add_entry((union v6addr *)TARGET_IP_V6, 0, 445566, 0, 0);
...
int policy_reject_response_v6_check(const struct __ctx_buff *ctx)
{
	...
	/* Should redirect ICMPv6 response back to interface */
	assert(*status_code == TC_ACT_REDIRECT);
	...
	/* Verify this is an ICMPv6 response packet */
	if (l3->nexthdr != IPPROTO_ICMPV6)
		test_fatal("expected ICMPv6 protocol, got %d", l3->nexthdr);

	/* Source should be swapped to target, destination should be client */
	if (memcmp(&l3->saddr, (void *)TARGET_IP_V6, 16) != 0)
		test_fatal("ICMPv6 src should be target IP");

	if (memcmp(&l3->daddr, (void *)CLIENT_IP_V6, 16) != 0)
		test_fatal("ICMPv6 dst should be client IP");

	icmp6 = (void *)l3 + sizeof(struct ipv6hdr);

	if ((void *)icmp6 + sizeof(struct icmp6hdr) > data_end)
		test_fatal("ICMPv6 header out of bounds");

	/* Verify ICMPv6 error type and code for policy rejection */
	if (icmp6->icmp6_type != ICMPV6_DEST_UNREACH)
		test_fatal("expected ICMPV6_DEST_UNREACH, got type %d", icmp6->icmp6_type);

	if (icmp6->icmp6_code != ICMPV6_ADM_PROHIBITED)
		test_fatal("expected ICMPV6_ADM_PROHIBITED, got code %d", icmp6->icmp6_code);

	/* Verify rate limiting map was updated */
	...
	struct ratelimit_value *rvalue;

	rvalue = map_lookup_elem(&cilium_ratelimit, &rkey);
	if (!rvalue)
		test_fatal("ratelimit map lookup failed");

	assert(rvalue->tokens > 0);
