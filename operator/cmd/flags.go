...
func (def operatorPprofConfig) Flags(flags *pflag.FlagSet) {
	...
	flags.Int(pprofBlockProfileRate, def.OperatorPprofBlockProfileRate, "Enable goroutine blocking profiling and set the rate of sampled events in nanoseconds (set to 1 to sample all events)")
}
