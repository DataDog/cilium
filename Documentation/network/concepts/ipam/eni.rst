...
``spec.eni.ena-queue-count``
  ...
  node, either ``auto`` or a positive number of queues. See
  :ref:`ipam_eni_ena_queues`.

  If unspecified, the number of queues is left to AWS.
...
The number of queues each ENI was attached with is reported per ENI in the
``CiliumNode`` resource, which makes the consumption of the budget visible:

.. code-block:: shell-session

    $ kubectl get ciliumnode ip-10-0-1-10 -o jsonpath='{range .status.eni.enis[*]}{.id}{"\t"}{.number}{"\t"}{.ena-queue-count}{"\n"}{end}'
