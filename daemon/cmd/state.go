...
func (r *endpointRestorer) checkEndpointLinkType(ep *endpoint.Endpoint) (string, error) {
...
func (r *endpointRestorer) validateDatapathModeCompatibility(endpoints map[uint16]*endpoint.Endpoint) error {
	...
	for _, ep := range endpoints {
		linkType, err := r.checkEndpointLinkType(ep)
		if err != nil {
		...
		if isIncompatible {
			epName := fmt.Sprintf("endpoint-%d", ep.ID)
...
func (d *Daemon) restoreOldEndpoints(state *endpointRestoreState) {
	...
	if err := r.validateDatapathModeCompatibility(r.restoreState.possible); err != nil {
		logging.Fatal(r.logger, "Datapath mode incompatibility detected", logfields.Error, err)
	}
