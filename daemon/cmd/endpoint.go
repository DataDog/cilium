...
func (d *Daemon) fetchK8sMetadataForEndpoint(nsName, podName string) (*slim_corev1.Pod, *endpoint.K8sMetadata, error) {
	log.Info("Anton-Test: fetchK8sMetadataForEndpoint: calling endpointMetadataFetcher.Fetch()")
	ns, p, err := d.endpointMetadataFetcher.Fetch(nsName, podName)
	if err != nil {
		return nil, nil, err
	}

	log.Info("Anton-Test: fetchK8sMetadataForEndpoint: calling k8s.GetPodMetadata()")
...
func (d *Daemon) createEndpoint(ctx context.Context, owner regeneration.Owner, epTemplate *models.EndpointChangeRequest) (*endpoint.Endpoint, int, error) {
	...
	if ep.K8sNamespaceAndPodNameIsSet() && d.clientset.IsEnabled() {
		ep.Logger("anton-test").Info("Anton-Test: createEndpoint: Calling handleOutdatedPodInformer")
		...
		if err != nil {
			...
			ep.Logger("anton-test").Info("Anton-Test: createEndpoint: handleOutdatedPodInformer finished, retrieved labels " + logfields.Repr(k8sMetadata.IdentityLabels))
			ep.SetPod(pod)
	...
	if len(identityLbls) == 0 {
		...
		ep.Logger("anton-test").Info("Anton-Test: createEndpoint: assigned init identity")
	}
	...
	if ep.K8sNamespaceAndPodNameIsSet() && d.clientset.IsEnabled() {
		...
		if _, k8sLabelsConfigured := identityLbls[k8sConst.PodNamespaceLabel]; !k8sLabelsConfigured {
			ep.Logger("anton-test").Info("Anton-Test: createEndpoint: calling RunMetadataResolver because no k8s labels are configured (2: non-blocking)")
...
func (d *Daemon) handleOutdatedPodInformer(
	...
	err = resiliency.Retry(ctx, 2*time.Second, 20, func(_ context.Context, _ int) (bool, error) {
		...
		log.WithFields(logrus.Fields{
			logfields.K8sPodName: ep.K8sNamespace + "/" + ep.K8sPodName,
		}).Info("Anton-Test: handleOutdatedPodInformer: calling fetchK8sMetadataForEndpoint")
		pod, k8sMetadata, err2 = d.fetchK8sMetadataForEndpoint(ep.K8sNamespace, ep.K8sPodName)
		log.WithFields(logrus.Fields{
			logfields.K8sPodName: ep.K8sNamespace + "/" + ep.K8sPodName,
		}).Info("Anton-Test: handleOutdatedPodInformer: finished fetchK8sMetadataForEndpoint")
		if ep.K8sUID == "" {
