...
func InitGlobalFlags(logger *slog.Logger, cmd *cobra.Command, vp *viper.Viper) {
	...
	flags.String(option.PolicyDenyResponse, defaults.PolicyDenyResponse, "How to handle egress traffic dropped by network policy: either drop the packet (\"none\") or reject with an ICMP Destination Unreachable (\"icmp\")")
