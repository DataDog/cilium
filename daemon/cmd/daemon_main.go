...
func InitGlobalFlags(logger *slog.Logger, cmd *cobra.Command, vp *viper.Viper) {
	...
	flags.String(option.PolicyDenyResponse, defaults.PolicyDenyResponse, "How to handle traffic dropped by policy: either drop the packet (\"drop\") or reject with an ICMP Destination Unreachable (\"reject\")")
